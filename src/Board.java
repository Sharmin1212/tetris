...
public class Board extends JPanel implements ActionListener {
    ...
    private boolean canMoveTo(int newRow, int newCol) {
        if ((newCol + currentShape.getXmin() < 0) || (newCol + currentShape.getXmax() >= NUM_COLS)) {
            return false;
        }
        return true;
    }
    ...
    private void drawCurrentShape(Graphics g) {
        ...
        for (int point = 0; point <= 3; point++) {
            drawSquare(g, currentRow + squaresArray[point][1], currentCol + squaresArray[point][0], Tetrominoes.SquareShape);
